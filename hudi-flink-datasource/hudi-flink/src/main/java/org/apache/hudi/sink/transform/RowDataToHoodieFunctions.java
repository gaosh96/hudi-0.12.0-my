...
public abstract class RowDataToHoodieFunctions {
  ...
  public static MyRowDataToHoodieFunction<RowData, HoodieRecord> create(RowType rowType, Configuration conf) {
    if (conf.getLong(FlinkOptions.WRITE_RATE_LIMIT) > 0) {
      return new MyRowDataToHoodieFunctionWithRateLimit<>(rowType, conf);
    } else {
      return new MyRowDataToHoodieFunction<>(rowType, conf);
    }
  }
}
